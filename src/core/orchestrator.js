...
import { inferCompanyAssignment } from '../shared/companyCatalog.js';
...
function resolveTaskAssignment(snapshot, input = {}) {
  const sourceTask = input.sourceTaskId
    ? snapshot.tasks.find((entry) => entry.id === input.sourceTaskId) ?? null
    : null;
  ...
  return inferCompanyAssignment(input, {
    sourceTask,
    leadAgentId: activeWorkspace?.leadAgentId,
  });
}
...
export class OperatorOrchestrator extends EventEmitter {
  ...
  createTask(input = {}) {
    const snapshot = this.snapshot();
    const assignment = resolveTaskAssignment(snapshot, input);
    const task = createTaskRecord({
      ...clone(input),
      ownerAgentId: input.ownerAgentId ?? assignment.ownerAgentId,
      departmentId: input.departmentId ?? assignment.departmentId,
    }, snapshot.settings, '');
  ...
  seedWorkspaceTask(templateId = null) {
    ...
    const task = createTaskRecord({
      ...
      ownerAgentId: refreshedTemplate.ownerAgentId,
      departmentId: refreshedTemplate.departmentId,
